...
extension BitboardTests {
  ...
  func test_subscript_file_rank() throws {
    ...
    bitboard.bitset(Bitboard9x9.Coordinate.init(x: 1, y:1))
  ...
  func test_subscript_coordinate() throws {
    var bitboard = Bitboard9x9.init()
    let coordinate1 = Bitboard9x9.Coordinate.init(x: 1, y:1)
    bitboard.bitset(coordinate1)
    
    let element1 = bitboard[coordinate1]
    XCTAssertTrue(element1)
    
    let coordinate2 = Bitboard9x9.Coordinate.init(x: 2, y:2)
  ...
  func test_iteration() throws {
    var bitboard = Bitboard9x9.init()
    let coordinates = [
      Bitboard9x9.Coordinate.init(x: 0, y:0),
      Bitboard9x9.Coordinate.init(x: 8, y:8)
    ]
    ...
    bitboard.makeIterator().enumerated().filter { $0.element == true }.forEach { value in
      let coordinate: Bitboard9x9.Coordinate = Bitboard9x9.index_to_coordinate(index: value.offset)
